# Social Square Family Fallback Refinement

## Visual previews
![Repair before](C:\Users\Fedelesh_dm\mvp\dataset\social-square\reports\contact-sheet-square-family-fallback-before.svg)

![Repair after](C:\Users\Fedelesh_dm\mvp\dataset\social-square\reports\contact-sheet-square-family-fallback-after.svg)

## What changed in logic
- square repair now treats some overlay-safe near-miss cases as overlay-exhausted and compares a structured-card fallback earlier
- square structured card is now allowed to compete with overlay rescue even when the case is not broadly structural, but is stuck on narrow safe-text misses
- thresholds remain unchanged; this pass only changes square family fallback behavior
...
- use this family fallback pass only as a square repair improvement, not as a threshold rewrite
